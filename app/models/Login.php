...
class Login extends Model{
    ...
    public function register($userId, $name, $email, $password, $confirmPassword, $workplace, $position, $contact_no, $worker_id, $group, $role, $activate){
        ...
        $query = 'INSERT INTO users (workerid, "name", "role", "group", department , hashed_password , email , is_email_activated, email_token, email_last_verification, position) ';
        $query .= 'VALUES (:workerid, :name, :role, :group, :department, :hashed_password, :email, :is_email_activated, :email_token, :email_last_verification, :position)';

        $database->prepare($query);
        $database->bindValue(':workerid', $worker_id);
        $database->bindValue(':name', $name);
        $database->bindValue(':email', $email);
        $database->bindValue(':role', $role);
        $database->bindValue(':group', $group);
        $database->bindValue(':department', $workplace);
        $database->bindValue(':hashed_password', $hashedPassword);
        $database->bindValue(':is_email_activated', $activate);
        ...
        $user_id = $database->lastInsertedId();

        $profile = "INSERT INTO users_profile (user_id, name, workplace_name, contact_no) VALUES (:user_id, :name, :workplace_name, :contact_no)";

        $database->prepare($profile);
        $database->bindValue(':user_id', $user_id);
        $database->bindValue(':name', $name);
        $database->bindValue(':workplace_name', $workplace);
        $database->bindValue(':contact_no', $contact_no);
    ...
    public function doLogIn($workerid, $password, $rememberMe, $userIp, $userAgent){
        ...
        $database->prepare("SELECT * FROM users WHERE workerid = :workerid AND is_email_activated = 1 LIMIT 1");
        ...
        if(!$validation->validate([
            ...
            if($this->checkIp($userIp, $user["role"])){
                ...
                $this->handleIpFailedLogin($userIp, $workerid);
            }
    ...
    private function isEmailExists($email){
        ...
        $database = Database::openConnection();
        $database->prepare("SELECT * FROM users WHERE email = :email AND is_email_activated = 1 ");
        $database->bindValue(':email', $email);
        $database->execute();
    ...
    public function isForgottenPasswordTokenValid($userId, $passwordToken){
        ...
        $database = Database::openConnection();
        $database->prepare("SELECT * FROM forgotten_passwords WHERE user_id = :user_id AND password_token = :password_token ");
        $database->bindValue(':user_id', $userId);
        $database->bindValue(':password_token', $passwordToken);
        $database->execute();
        $forgottenPassword = $database->fetchAssociative();
