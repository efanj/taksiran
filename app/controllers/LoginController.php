...
class LoginController extends Controller {
    ...
    public function beforeAction(){
        ...
        switch($action){
            case "register":
                $this->Security->config("form", [ 'fields' => ['email', 'name', 'workplace', 'position', 'contact_no', 'worker_id', 'group', 'password', 'confirm_password', 'role', 'activate']]);
                break;
            case "login":
                $this->Security->config("form", [ 'fields' => ['workerid', 'password'], 'exclude' => ['remember_me', 'redirect']]);
                break;
            case "forgotPassword":
    ...
    public function register(){
        ...
        $workplace          = $this->request->data("workplace");
        $position           = $this->request->data("position");
        $contact_no         = $this->request->data("contact_no");
        $worker_id          = $this->request->data("worker_id");
        ...
        $result = $this->login->register(Session::getUserId(), $name, $email, $password, $confirmPassword, $workplace, $position, $contact_no, $worker_id, $group, $role, $activate);
    ...
    public function login(){
        ...
        $workerid    = $this->request->data("workerid");
        $password    = $this->request->data("password");
        $rememberMe  = $this->request->data("remember_me");
        $redirect    = $this->request->data("redirect");

        $result = $this->login->doLogIn($workerid, $password, $rememberMe, $this->request->clientIp(), $this->request->userAgent());
