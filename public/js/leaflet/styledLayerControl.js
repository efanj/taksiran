L.Control.StyledLayerControl = L.Control.Layers.extend({
    ...
    for (group in this._groupList) {
      if (this._groupList[group].groupName == group_Name) {
        for (layer in this._layers) {
          if (
            this._layers[layer].group &&
            this._layers[layer].group.name == group_Name
          ) {
            if (del) {
  ...
  _initLayout: function () {
    ...
    for (
      var c = 0;
      c <
      (containers = container.getElementsByClassName("ac-container")).length;
      c++
    ) {
  ...
  _on_resize_window: function () {
    ...
    for (var c = 0; c < containers.length; c++) {
      ...
      containers[c].style.maxHeight =
        window.innerHeight - 90 < this._removePxToInt(this._default_maxHeight)
          ? window.innerHeight - 90 + "px"
          : this._removePxToInt(this._default_maxHeight) + "px"
    }
  },
  ...
  _checkIfDisabled: function (layers) {
    ...
    for (layerId in this._layers) {
      if (
        this._layers[layerId].layer.options &&
        (this._layers[layerId].layer.options.minZoom ||
          this._layers[layerId].layer.options.maxZoom)
      ) {
        var el = document.getElementById(
          "ac_layer_input_" + this._layers[layerId].layer._leaflet_id
        )
        if (
          currentZoom < this._layers[layerId].layer.options.minZoom ||
          currentZoom > this._layers[layerId].layer.options.maxZoom
        ) {
  ...
  _addItem: function (obj) {
    ...
    if (!groupContainer) {
      ...
      var s_type_exclusive = this.options.exclusive
        ? ' type="radio" '
        : ' type="checkbox" '
      ...
      inputElement =
        '<input id="ac' +
        obj.group.id +
        '" name="accordion-1" class="menu" ' +
        s_expanded +
        s_type_exclusive +
        "/> "
      inputLabel =
        ' <label for="ac' + obj.group.id + '">' + obj.group.name + "</label>"
      ...
      if (obj.overlay && this.options.group_togglers.show) {
        // Toggler container
        var togglerContainer = L.DomUtil.create(
          "div",
        ...
        if (L.Browser.touch) {
          L.DomEvent.on(linkAll, "click", L.DomEvent.stop).on(
            linkAll,
            "click",
            this._onSelectGroup,
            this
        ...
        // Separator
        var separator = L.DomUtil.create(
          "span",
        ...
        if (L.Browser.touch) {
          L.DomEvent.on(linkNone, "click", L.DomEvent.stop).on(
            linkNone,
            "click",
            this._onUnSelectGroup,
            this
          )
        } else {
          L.DomEvent.on(linkNone, "click", L.DomEvent.stop).on(
        ...
        if (
          obj.overlay &&
          this.options.group_togglers.show &&
          obj.group.removable
        ) {
          // Separator
          var separator = L.DomUtil.create(
        ...
        if (obj.group.removable) {
          // Link delete group
          var linkRemove = L.DomUtil.create(
            "a",
            "group-toggle-none",
            togglerContainer
          )
          ...
          if (L.Browser.touch) {
            L.DomEvent.on(linkRemove, "click", L.DomEvent.stop).on(
              linkRemove,
              "click",
              this._onRemoveGroup,
              this
